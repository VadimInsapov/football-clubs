<x-app-layout>
    ...
    @can('add-club', $currentUser)
        <x-menu>
            <a class="button menu__item button--theme-green" href={{route('users.index')}}>Пользователи</a>
            <a class="button menu__item button--theme-green" href={{route('create')}}>Добавить клуб</a>
            <a class="button menu__item button--theme-green" href={{route('user.ribbon', ['userId' => Auth::user()->id])}}>Лента</a>
        </x-menu>
    @else
        <x-menu>
            <a class="button menu__item button--theme-green" href={{route('users.index')}}>Пользователи</a>
    ...
    <div class="clubs-list">
        @forelse($clubs as $club)
            <x-club>
                ...
                @endif
                <x-club__info>
                    <x-slot name="title"><a class="link"
                                            href={{route('show', ['clubId' => $club->id])}}>{{$club->name}}</a></x-slot>
